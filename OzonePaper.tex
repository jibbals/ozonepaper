...
%\section{Introduction}
  ...
  Models show stratospheric ozone depletion has propagated to the upper troposphere \citep{Stevenson2013}. 
  ...
  A big influence on the high surface ozone concentrations over the eastern Mediterranean (EM) can be attributed to stratospheric mixing and anticyclonic subsidence \citep{Zanis2014, Akritidis2016}.
  The EM frequently shows a summer maximum of stratospheric subsidence, which sits between 20$^{\circ}$E and 35$^{\circ}$E and 31$^{\circ}$N to 39$^{\circ}$N, while zonally most subtropical ozone folds occur during Winter \citep[][and references therein]{Tyrlis2014}.
  ...
  % TODO: small review of transport literature
  ...
  The long range transport of enhanced ozone can be facilitated by shear upper tropospheric winds, with remarkably little convective mixing, as shown by \cite{Trickl2014} who measure STT air masses two days and thousands of kilometres from their source.
  ...
  Section 3 provides our newly derived climatology of STT frequency, seasonality, intrusion altitude, and depth, and Section 4 uses these new climatologies to evaluate upper tropospheric ozone in a global chemical transport model (GEOS-Chem). 
...
\section{Data and Methods}
  ...
  \label{sec:ozonesondes}
    ...
    For this study, we use the 2004-2013 data for Melbourne and Macquarie Island and the 2006-2013 data for Davis because both ozone and geopotential height (GPH) profiles were measured during these periods.
    ...
    \end{enumerate}
    The ozone tropopause may misdiagnose the real tropopause altitude during stratosphere-troposphere exchange; however, it is useful at polar latitudes in winter, where the lapse-rate definition may result in artificially high tropopause values \citep{Bethan1996, Tomikawa2009, Alexander2013}.
    We require lapse rate tropopauses to be at a minimum of 4~km altitude. %This is due to several temperature inversions causing low misdiagnosed tropopauses.
    Another commonly used tropopause definition (the dynamical tropopause) is determined from the potential vorticity unit (PVU; 1~PVU$= 10^{-6}$~m$^2$ s$^{-1}$ K kg$^{-1}$). In the extra-tropics the isosurface where PVU$=2$ is often used, which allows a 3D view of folds and other tropopause features in a sufficiently resolved model \citep{Skerlak2014,Tyrlis2014}.
    In this work, the ozone tropopause is used when determining STT events or measured tropopause altitude.
    ...
    % figure produced in seasonal_tropopause in Examine_stations.py
      ...
      \caption{%
	Multi-year monthly median tropopause altitude (using ozone defined tropopause) determined from ozonesondes measurements at Melbourne (2004-2013), Macquarie Island (2004-2013) and Davis (2006-2013) (solid lines).
    ...
    The seasonality shown in Fig. \ref{fig:seasonaltropozone} is consistent with remote free tropospheric photochemistry determined by solar radiation availability and temperature, resulting in higher ozone in winter \citep{Lelieveld2000}.
    NO$_2$ stratospheric observations have been conducted in the Southern hemisphere at Lauder, Macquarie island and Arrival Heights i.e. \citet{Struthers2004} which displays a winter minima in seasonality consistent with an ozone maxima.
  ...
  \label{sec:WeatherClassifications}
    ...
    The stratospheric polar vortex may create ozone folds without other sources of upper tropospheric turbulence \citep[e.g.][]{Baray2000,Sprenger2003,Tyrlis2014}
    ...
    These are included in a supplementary document (TODO: hard-code reference).
...
\section{STT event climatologies}
  ...
  This seasonality is not clear in the recent ERA-Interim tropopause fold analysis performed by \citet{Skerlak2015}, where a winter maximum of ozone fold frequency ($\sim 0.5\%$ more folds in winter) over Australia can be seen to the north of Melbourne.
  Their work seems to show slightly higher fold frequencies over Melbourne in summer \citep[][Fig. 5]{Skerlak2015}, which agrees with our ozonesonde measurements.
  Their winter maximum is in the subtropics only - from around 20$^{\circ}$~S to 40$^{\circ}$~S, which can be seen as the prevalent feature over Australia in Fig. 5 of their publication.
...
\section{Simulation of southern regions of ozone columns}
  ...
  The model is only partially able to reproduce the variability in the observations, with paired r$^2$ values for of 0.38 for Davis, 0.18 for Macquarie Island, and 0.37 for Melbourne.
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \subsection{Results}
    ...
    \begin{figure}
      % Figure from flux_summary.pro - combined using pinta
    ...
    The uncertainty (shaded) is quite high, on the order of $100\%$ (see Sect. \ref{sec:fluxuncertainty}), this level of uncertainty means we do not attempt to draw conclusions about the seasonal quantities of STT ozone flux, but calculate the annual amount based on the sum of the monthly values.
  ...
  \subsection{Comparison to literature}
    ...
    \citet{Olsen2003} used PV and winds from the GEOS reanalysis combined with ozone measurements from the TOMS satellite to estimate that the ozone flux between 30$^{\circ}$ S and 60$^{\circ}$ S 210~Tg yr$^{-1}$, with the maximum occurring over SH winter.
    ...
    As mentioned in Sect. \ref{sec:eventclimatologies}, the maximum STT influx which occurs during SH winter is almost entirely due to the dominant STT system which occurs annually over the southern Indian ocean and middle of Australia.
...
\label{sec:sensitivity}
  ...
  \subsection{Flux calculations}
    ...
    Small changes in the region don't have a large affect on the per area flux calculations, with an increasing or decreasing the regions by 1$^{\circ}$ on each side ($\sim 10\%$ change in area) changing the resulting flux by $\sim 1\%$~Kg cm$^{-2}$.
...
Estimating STT flux using ozonesonde data alone remains challenging; however, the very high vertical resolution provided by ozonesondes means they are capable of detecting STT events that models, re-analyses, and satellites may not. 
