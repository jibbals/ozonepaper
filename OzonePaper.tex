...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \begin{figure}[t]
  % Comparison region made in map script
  ...
  The region containing Melbourne and Macquarie (M region) describes $\sim 5.87$ million square km, extending from 60$^{\circ}$ S to 35$^{\circ}$ S, and 140$^{\circ}$ E to 165$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 3.3 \times 10^{17}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 1.6~Tg a$^{-1}$.
  The region containing Davis (D region) describes $\sim 2.91$ million square km, extending from 70$^{\circ}$ S to 55$^{\circ}$ S, and 60$^{\circ}$ E to 90$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 2.7 \times 10^{17}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 0.6~Tg a$^{-1}$. TODO: UPDATE THESE NUMBEROONIES.
  ...
  To determine the ozone column attributable to STT, we determine monthly averaged STT impact (\textit{I}) and the monthly mean tropospheric ozone column over the Southern Ocean (from the GEOS-Chem multi-year mean, $\Omega_{SO_{O_3}}$), expressed simply as flux$_i = \Omega_{SO_{O_3}} \times I$.
  Where flux$_i$ is the STT flux per event in each month.
  ...
  This leads us to multiply our flux$_i$ by L, and then by the range dictated by our assumed event lifetime.
  
  Figures \ref{fig:MExtrapolation, fig:DExtrapolation} (upper panel) shows the factors I, L, and $\Omega_{SO_{O_3}}$ which are used along with the assumed event lifetime to estimate the STT flux.
  ...
  Fig. \ref{fig:MExtrapolation, fig:DExtrapolation} (lower panel) shows the results of the calculation when we choose 2.5 days for our flux estimation, with the range shown in representing the values calculated if we assume events last 1 day (upper bound of estimated flux) or one week (lower bound of estimated flux).
  ...
  \begin{figure}[t]
    ...
    \caption{%
      ...
      The tropospheric ozone column $\Omega_{trop O_3}$ (black, left axis) is from GEOS-Chem, while the STT likelihood $L$ (magenta, right axis) and impact $I$ (teal, right axis) are from the ozonesonde measurements.
  ...
  Skerlak2014 estimate see around X to Y kg/km2/month in these regions...
  
  \cite{Skerlak2014} shows an estimate of roughly X to Y~kg km$^{-2}$ month$^{-1}$ in these regions, while we estimate X to Y~kg km$^{-2}$ month$^{-1}$ with our technique TODO:.
  The seasonal contributions are shown in Table TODO:
  \begin{table}[t]
    \caption{Seasonal ozone contribution in our two regions, in kg km$^{-2}$ month$^{-1}$.}
    \begin{tabular}{ l l l l l } 
      \hline
      Region & DJF & MAM & JJA & SON \\
      \hline
      M region & X(X\%) & X(X\%) & X(X\%) \\ 
      D region & X(X\%) & X(X\%) & X(X\%) \\
      \hline
    \end{tabular}
    \label{table:extrapolationResults}
  \end{table}
  
  TODO: uncertainties with extrapolations discuss overall pct uncertainty after bracketting range
  ...
  The overall uncertainty as a percentage is shown in parentheses in Table \ref{table:extrapolationResults}.
  ...
  The results over the southern ocean can be viewed in the supplementary section (TODO: hardcode reference here{sup:sec:SOExtrapolation}).
