...
\section{Data and Methods}
  ...
  \subsection{Characterisation of STT events and associated fluxes}
    ...
    \begin{figure}[t]
      % Figure 4 created in getevents.pro, edited in inkscape
...
\section{Simulation of southern regions of ozone columns}
  ...
  Recently (TODO: cite Hu et al., 2017) examined GEOS-Chem ozone simulations and saw a similar overestimation of upper troposphere ozone in the mid southern latitudes when running with GEOS5 meteorological fields. 
  They suggest that in general GEOS-Chem underestimates STT, with $\sim 360$~Tg a$^{-1}$ simulated globally, compared to $\sim 550$~Tg a$^{-1}$ observationly constrained.
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  \subsection{Method}
    ...
    The regions thus describe $\sim 4.4$, $4.6$, and $4.8$ million square km, for Davis, Macquarie Island, and Melbourne respectively.
    ...
    To determine the ozone column attributable to STT, we determine monthly averaged STT impact (\textit{I}; fraction of tropospheric ozone sourced from the stratosphere) and the monthly mean tropospheric ozone column (from the GEOS-Chem multi-year mean, $\Omega_{Trop_{O_3}}$) over the regions described above.
    ...
    The monthly ($P$)robability of STT is calculated from fraction of ozonesonde releases for which an STT event was detected, within each month.
    ...
    This leads us to multiply our flux$_i$ by $P$, and then by the term $M$ ($M=$ days per month $/N$) determined by our assumed event lifetime in order to determine monthly STT ozone flux.
	...
	The tropospheric ozone column $\Omega_{Trop_{O_3}}$ (black, left axis) is from GEOS-Chem, while the STT probability  $P$(magenta, right axis) and impact $I$ (teal, right axis) are from the ozonesonde measurements.
	The STT impact is multiplied by four to better show the seasonality.
	(Bottom) Estimated contribution of STT to tropospheric ozone columns over the region, with range determined by the uncertainty estimated as outlined in Sect. \ref{sec:uncertainty}.
	The black line shows STT ozone flux if event lifetime is assumed to be two days, with dashed lines showing flux if event lifetime was assumed to be either one day or one week.}
    ...
    \begin{table}[t]
      ...
      \caption{Seasonal STT ozone contribution in the regions near each site, in kg km$^{-2}$ month$^{-1}$.}
      \begin{tabular}{ l l l l l } 
	\hline
	Region & DJF & MAM & JJA & SON \\
	\hline
	Davis       & 34.9(  188\%) & 21.8(  225\%) & 19.4(  170\%) & 27.7(  165\%) &  \\ 
	Macqaurie   & 52.9(  181\%) & 20.4(  267\%) & 10.7(  345\%) & 13.0(  328\%) &  \\
	Melbourne   &111.5(  144\%) & 38.4(  210\%) & 13.9(  297\%) & 37.8(  242\%) & \\
	\hline
      \end{tabular}
    ...
    The lower panel of these two figures show the results of the calculation when we choose 2 days for our flux estimation, with dotted lines showing the values calculated if we assume events last 1 day (upper bound of estimated flux) or one week (lower bound of estimated flux).
    ...
    In these regions with high wind shear, mixing appears to be slower, which allows ozone intrusions to be transported further without dissipating into the troposphere.
  
  \subsection{Results and comparisons}
    \cite{Skerlak2014} show an estimate of roughly 40 to 150~kg km$^{-2}$ month$^{-1}$ in these regions, over all seasons (see Fig. 16, 17 in their publication) while we estimate 4 to 20~kg km$^{-2}$ month$^{-1}$ STT impact, except for Summer (DJF) over Melbourne, which shows a large spike to 195~kg km$^{-2}$ month$^{-1}$. 
    ...
    \cite{Roelofs1997} use a model with a stratospheric ozone tracer to estimate STT impacts, similar to our model they see higher SH tropospheric ozone concentrations in the winter, however the peak of SH STT occurence is in winter.
    ...
    The disagreements largely reflect the difference between point source based estimates and zonally averaged estimates, as the meteorological behaviour at our three sites is not the same as the southern hemisphere in general.
    As mentioned in Sect. \ref{sec:eventclimatologies}, the maximum in SH winter when examined on a large scale is almost entirely due to the dominant STT system which occurs anually over the southern Indian ocean and middle of Australia.
  ...
  \label{sec:uncertainty}
    ...
    For each term; $I$, $P$, and $\Omega_{O_3}$, the monthly averaged quantities are stored for each year where we have data, and then the standard deviation over the 7-10 values for each month are used to calculate the proportional uncertainty for each parameter.
    ...
    The overall uncertainty as a percentage is shown in parentheses in Table \ref{table:extrapolationResults}, these values are on the order of 200\%, largely due to uncertainty in the $P$ and $I$ factors which both sit near 100\% uncertainty for each month.
    The uncertainty in the $I$ factor of our STT flux calculation is estimated from the standard deviation of the calculated values, for each of the three sites, as the variability in this term appears to be homoskedastic (unvarying in time).
    The $P$ factor is assumed to follow a Bernoulli distribution, with variance calculated for each month at each site.
    Uncertainty in tropospheric ozone from GEOS-Chem is estimated using the standard deviation at each site, this term is also seen to be homoskedastic. 
    Uncertainty in assumed event lifetime is set at 50\%.
    These uncertainties propogate to the STT flux calculation through the standard variance of a product of independent variables calculation: $var(\Pi_i X_i) = \Pi_i ( var(X_i) + E(X_i)^2 ) - (\Pi_i E(X_i))^2$
     
    Considering the $I$ factor, as discussed in here and in Sect. \ref{sec:sensitivity}, there are several uncertainties in our method that are likely to lead to a low bias, including a conservative estimate of flux within each event and a process that excludes potential events which are too close to the tropopause or to transported smoke plumes.
