...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \subsection{Results}
    ...
    The uncertainty (shaded) is quite high, on the order of $100\%$, this level of uncertainty means we do not attempt to draw conclusions about the seasonal quantities of STT ozone flux, but calculate the anual amount based on the sum of the monthly values.
...
\label{sec:sensitivity}
  \subsection{Event detection}
    ...
    We use the 95th percentile because at this point the filter locates clear events with fewer than 2\% obvious false positive detections.
    TODO: update these numbersEvent detection is sensitive to this choice; for example, using the 96th, and 97th percentile instead decreased detected events by 2, 9 (2,10\%) at Davis, 13, 31 (11, 28\%) at Macquarie Island, and 8, 24 (6, 18\%) at Melbourne.
  ...
  \subsection{Flux calculations}
   ...
   Without removing smoke flagged events we see an increase in estimated yearly flux of $\sim 1.1, 1.4 \times 10^17 $~molecules cm^{-2} yr^{-1} (which is a change of $\sim 15, 20%$), over Macquarie Island and Melbourne respectively.
