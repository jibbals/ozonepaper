...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  \subsection{Method}
   ...
   If we assume events last N days, we find how many events per month by multiplying the days in a month by P and dividing by the assumed event lifetime.
    ...
    This leads us to multiply our flux$_i$ by P, and then by the term (M) determined by our assumed event lifetime in order to determine monthly STT ozone flux.
    ...
    \begin{figure}
      ...
      \caption{%
				...
				Black line shows STT ozone flux if event lifetime is assumed to be 2 days, with shaded region showing the large uncertainty in this calculation, estimated as outlined in Sect \ref{Section:uncertainty}.}
    ...
    The upper panels in figures \ref{fig:MExtrapolation}, and \ref{fig:DExtrapolation} show the factors I, P, and $\Omega_{SO_{O_3}}$ which are used along with the assumed event lifetime to estimate the STT flux.
  ...
  \subsection{Results and comparisons}
    \cite{Skerlak2014} shows an estimate of roughly 40 to 150~kg km$^{-2}$ month$^{-1}$ in these regions, over all seasons (see Fig. 16, 17 in their publication) while we estimate 2 to 41~kg km$^{-2}$ month$^{-1}$ STT impact, of which the largest part is in Summer (DJF).
    %, of which 0 to 10~kg km$^{-2}$ month$^{-1}$ enters the boundary layer 
