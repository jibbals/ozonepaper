...
\section{Data and Methods}
  ...
  \label{sec:ozonesondes}
    ...
    Another commonly used tropopause definition (the dynamical tropopause) is determined from the potential vorticity unit (PVU; 1~PVU$= 10^{-6}$~m$^2$ s$^{-1}$ K kg$^{-1}$). In the extratropics the isosurface where PVU$=2$ is often used, which allows a 3D view of folds and other tropopause features in a sufficiently resolved model \citep{Skerlak2014,Tyrlis2014}.
  ...
  \label{sec:WeatherClassifications}
    ...
    These are included in a supplementary document (TODO: hardcode reference).
...
\section{STT event climatologies}
  ...
  Over Melbourne, the event depth is more spread out, with peak ozone enhancement generally occuring up to 6~km below the tropopause.
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \label{sec:fluxcalc}
    ...
    Observed tropospheric columns are calculated from the ozonesondes by calculating the ozone number density (molec cm$^{-3}$) using measured ozone partial pressure (P$_{O_3}$) and integrating vertically up to the tropopause:
  ...
  \subsection{Results}
    ...
    The uncertainty (shaded) is quite high, on the order of $100\%$ (see Sect. \ref{sec:fluxuncertainty}), this level of uncertainty means we do not attempt to draw conclusions about the seasonal quantities of STT ozone flux, but calculate the anual amount based on the sum of the monthly values.
  ...
  \subsection{Comparison to literature}
	...
	Macqaurie   & 61.3(   85\%) & 70.7(   91\%) & 17.9(  139\%) &  7.7(  229\%) \\
...
\label{sec:sensitivity}
  \subsection{Event detection}
    ...
    An example of a well-defined event that occurs within 1~km of the tropopause is shown in the supplementary (Fig. TODO: hardcode reference here).
  ...
  \subsection{Flux calculations}
    ...
    Small changes in the region dont have a large affect on the per area flux calculations, with an increasing or decreasing the regions by 1$^{\circ}$ on each side ($\sim 10\%$ change in area) changing the resulting flux by $\sim 1\%$~Kg cm$^{-2}$.
