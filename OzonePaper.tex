...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \label{sec:fluxcalc}
    ...
    The uncertainty is quite high, on the order of $100\%$, except for two months over Macquarie Island where there were no events detected and therefor no estimated flux.
  ...
  \label{sec:uncertainty}
    ...
    Other possibly important uncertainties in our calculation of STT flux which we don't cover due to difficulty of calculation and analysis are listed here.
    Uncertainty in the $P$ parameter, which could be affected by false negatives caused by overzealous smoke plume filtering leading to underestimation.
    Filtering events which occur within 500~m of the tropopause may also lead to more false negatives, as well as lower impact estimates due to only measuring ozone enhancements which have descended and potentially slightly dissipated.
    The estimated tropospheric ozone columns modelled by GEOS-Chem may be biased, for instance Hu et al., 2017 suggest that in general GEOS-Chem (with GEOS-5 met. fields) underestimates STT, with $\sim 360$~Tg a$^{-1}$ simulated globally, compared to $\sim 550$~Tg a$^{-1}$ observationly constrained.
