...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  The region containing Melbourne and Macquarie (M region) describes $\sim 5.87$ million square km, extending from 60$^{\circ}$ S to 35$^{\circ}$ S, and 140$^{\circ}$ E to 165$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 2.3 \times 10^{17}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 1.1~Tg a$^{-1}$.
  The region containing Davis (D region) describes $\sim 2.91$ million square km, extending from 70$^{\circ}$ S to 55$^{\circ}$ S, and 60$^{\circ}$ E to 90$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 1.9 \times 10^{17}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 0.4~Tg a$^{-1}$. TODO: UPDATE THESE NUMBEROONIES.
  ...
  These assumptions are not realistic (the first one especially), however they allow a simple estimate of events per month from the relatively sparse dataset.
