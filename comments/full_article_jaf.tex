...
\section{Data and Methods}
  ...
  \label{Section:ozonesondes}
    ...
    Ozonesondes provide a profile of ozone mixing ratio, temperature, pressure, and humidity with high vertical resolution from the surface to 35 km. The ozone mixing ratio is quantified with an electrochemical concentration cell that senses the proportional electrical current from reaction of ozone with a solution of potassium iodide.
    ...
    In this work, the lower of these two tropopause altitudes is used. %, as both are calculated for each ozonesonde release. %% that's obvious if you are comparing two!
...
%    For each site, a vertical profile of ozone is stored every 6 hours, based on standard UTC+0 time.
  ...
  \subsection{Sensitivities and limitations}
    ...
    Event detection is highly sensitive to this choice; for example, using the 98.5th percentile instead increased detected events by 24 (33\%) at Melbourne, 19 (40\%) at Macquarie Island, and 10 (22\%) at Davis.
    ...
    The event detection was not as sensitive to the choice of Fourier bandpass scales: a widening of the allowed scales to the range 0.4-5.1 increased the detected events by 3 at Melbourne and 2 at Macquarie Island, while Davis lost two events (there were more events detected, but also more filtered out).
...
\section{STT event climatologies}
  ...
  \begin{figure}[!htbp]
    ...
    \caption{Seasonal distribution of the alternative STT proxy, obtained from consideration of the static stability at the ozone and lapse rate tropopauses, for Melbourne (1999-2012), Macquarie Island (1994-2012) and Davis (2003-2012).
...
\section{Simulation of southern mid-latitude ozone columns}
  ...
  Observed tropospheric columns are calculated from the ozonesondes by calculating the ozone number density (molec cm$^{-3}$) using measured ozone partial pressure (P$_{HCHO}$) and integrating over the measured GPH up to the tropopause:
  \begin{equation*}
  ...
  GEOS-Chem provides a reasonable simulation of the observed seasonality and magnitude of $\Omega_{O_3}$. Reduced major axis regression of observed versus simulated $\Omega_{O_3}$ gives a line of best fit with a slope near one-to-one for all sites, with the most divergent site (Melbourne) having a slope of 1.33. The model is only partially able to reproduce the variability in the observations, with paired r$^2$ values for of 0.38 for Davis, 0.18 for Macquarie, and 0.37 for Melbourne.
  Much of the variability is driven by the seasonal cycle, and after removing this effect (by subtracting the multi-year monthly means), the r$^2$ values decrease to .07, .12, .30 respectively, although the slope of all the regressions remains within 10\% of 1.
