...
%DIF ADD OzonePaper.tex                Tue Mar 28 13:46:00 2017
%% ---------------------------------
...
\DIFaddbegin 
  ...
  \label{Section:ozonesondes}
    ...
    \DIFaddbegin \DIFadd{Melbourne, a major city in Victoria, Australia is in the far south eastern section of the Australian mainland, actual releases are north of the central business district in the Broadmeadows suburb.
    ...
    \DIFaddend For this study, we use the 2004-2013 data for Melbourne and Macquarie and the 2006-2013 data for Davis because both ozone and geopotential height (GPH) profiles were measured during these periods.
    ...
    \DIFaddbegin \DIFadd{Another commonly used tropopause definition (the dynamical tropopause) is determined from the $\pm 2$ PVU isosurface, which allows a 3D view of folds and other tropopause features in a sufficiently resolved model \mbox{%DIFAUXCMD
\citep{Skerlak2014}
}%DIFAUXCMD
.
    }\DIFaddend 
  ...
  \subsection{Model description}
    ...
    \DIFaddbegin \DIFadd{The vertical resolution is finer near the surface at $\sim 60$~m between levels, spreading out to $\sim 500$~m near 10~km altitude, and reaching $\sim 1500$~m near the top of the atmosphere.
    ...
    \DIFaddbegin \DIFadd{GEOS-Chem uses the tropopause height provided by GEOS-5 met fields which are calculated using a lapse-rate definition using the first minimum above the surface in the function $0.03 \times T(p) - log_{10}(p)$, with p in hPa and T being temperature as a function of pressure \mbox{%DIFAUXCMD
\citep{Rienecker2008}
}%DIFAUXCMD
.
    }\DIFaddend 
  ...
  \subsection{Characterisation of STT events and associated fluxes}
    ...
    \DIFadd{We assume that in the troposphere, the ozone profile is a simple summation of sine and cosine waves.
    ...
    This is done quickly using a fast fourier transform method \mbox{%DIFAUXCMD
\citep{Press1992}
}%DIFAUXCMD
, using IDL code written by Dr. Simon Alexander shown in a supplementary document (TODO: how to reference?).
    }
    ...
    For an event to qualify as STT, a clear increase above the background ozone level is needed, and \DIFdelbegin \DIFdel{we find that }\DIFdelend a bandpass filter \DIFdelbegin \DIFdel{removes seasonal-scale }\DIFdelend \DIFaddbegin \DIFadd{leaves us with these increases minus any noise or seasonal scale vertical profile }\DIFaddend effects.
  ...
  \label{Section:BiomassBurning}
    ...
    All of the flagged events except for one (in January at Macquarie Island) occurred during the \DIFdelbegin \DIFdel{Southern Hemisphere }\DIFdelend \DIFaddbegin \DIFadd{SH }\DIFaddend burning season (July to December, \citet{Edwards2006}). % burning season from \citep{Pak2003}.
...
\section{STT event climatologies}
  ...
  \DIFaddend This seasonality is not seen in the recent ERA-Interim tropopause fold analysis performed by \citet{Skerlak2015}, where a winter maximum over Australia can be seen\DIFdelbegin \DIFdel{(although }\DIFdelend \DIFaddbegin \DIFadd{.
  However their winter maximum is }\DIFaddend in the subtropics only - from around 20$^{\circ}$~S to 40$^{\circ}$~S\DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{.
  \mbox{%DIFAUXCMD
\cite{Wauben1998}
}%DIFAUXCMD
...
examine modelled STT folds using ECMWF output over March 2000 - April 2001, and show that for this year there is a clear Austral winter maximum, again over the 20$^{\circ}$~S to 40$^{\circ}$~S band.
  The winter maximum }\textbf{\DIFadd{does not}} \DIFadd{include Melbourne, or the southern ocean, which may help explain why we see a seasonality which disagrees with these prior studies.
  }

  \DIFadd{The measurement sites are not in the regions which have a clear winter maximum seen in Fig. 1 of \mbox{%DIFAUXCMD
\cite{Sprenger2003}
}%DIFAUXCMD
...
\section{Simulation of southern mid-latitude ozone columns}
  ...
  \DIFaddbegin \DIFadd{GEOS-Chem outputs ozone density (molecules cm$^{-3}$), and height of each simulated box, as well as which level contains the tropopause, allowing modelled $\Omega_{O_3}$ to be calculated as the product of density and height summed up to the box below the tropopause level.
  }\DIFaddend In both observations and model, the maximum ozone column at Melbourne occurs in austral summer, with a minimum in winter, while Macquarie and Davis show the opposite seasonality.
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \DIFdelbegin \DIFdel{We use simulated tropospheric ozone columns from GEOS-Chem to extrapolate the ozonesonde-based estimates to the entire Southern Ocean region, defined here as 35$^{\circ}$ S-75$^{\circ}$ S to encompass our three measurement sites. To do so, we multiply the monthly likelihoods of STTs (fraction of ozonesonde releases for which an STT event was detected, per month, }\textit{\DIFdel{f}}%DIFAUXCMD
\DIFdel{)
  by the monthly mean fraction of the ozone column attributed to STT (as in Fig. \ref{fig:fluxsummary}, }\textit{\DIFdel{I}}%DIFAUXCMD
\DIFdel{) and by the monthly mean tropospheric ozone column over the Southern Ocean (from the GEOS-Chem multi-year mean, $\Omega_{SO_{O_3}}$), expressed simply as Flux$= \Omega_{SO_{O_3}} \times f \times I$.
  The monthly values of each term in this equation }\DIFdelend \DIFaddbegin \DIFadd{TODO: mention supplementary extrapolation. and add plots for subregion extrap (perhaps?)
  TODO: Compare against Skerlak values
  Two regions are used to examine possible STT flux over a larger area using modeled tropospheric ozone concentrations.
  The regions }\DIFaddend are shown in Fig. \DIFdelbegin \DIFdel{\ref{fig:SOExtrapolation} (upper panel).
  A more spatially resolved estimate could be determined by dividing the Southern Ocean region into longitudinal and latitudinal bins for calculating the average $\Omega_{SO_{O_3}}$ from GEOS-Chem, applying latitudinal gradients to $f$ and $I$ based on their values at the three sonde release sites, and adding longitudinal variability due to seasonal stratospheric wind jet streams \mbox{%DIFAUXCMD
...
, but this is beyond the scope of this work and in any case would not address the limitations of the estimate provided below.
...
}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:ComparisonRegion}.
  The region containing Melbourne and Macquarie describes $\sim 5.87$ million square km, extending from 60$^{\circ}$ S to 35$^{\circ}$ S, and 140$^{\circ}$ E to 165$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 3.977 \times 10^{16}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 0.1862~Tg a$^{-1}$.
  %DIF > If we assume the proportion of ozone due to an event is actually 35\%, as in Terao08 for the northern hemisphere: 4.627e+17 molecules/cm2/yr (2.166e+00 Tg/yr)
  The region containing Davis describes $\sim 2.91$ million square km, extending from 70$^{\circ}$ S to 55$^{\circ}$ S, and 60$^{\circ}$ E to 90$^{\circ}$ E, with an estimated STT ozone contribution of $\sim 2.343 \times 10^{16}$ molecules cm$^{-2}$ a$^{-1}$ or equivalently 0.05440~Tg a$^{-1}$.
  }\DIFaddend 
  ...
  %DIF >  Flux box and whisker plots from flux_summary.pro in OzoneWork(HPC) and edited in Inkscape
    ...
    \caption{\DIFaddFL{Ozonesonde release sites and the regions used to examine STT effect on tropospheric ozone levels.}}
  ...
  Previous }\DIFdelend \DIFaddbegin \DIFadd{TODO: Discuss other literature's estimates of STT flux comparing it with small regions.
  Skerlak2015
  }

  \DIFadd{Two previous }\DIFaddend studies have found STT ozone fluxes in the SH extratropics are largest from autumn or winter to early spring\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
%DIFDELCMD <   %%%
\DIFdel{Summing the monthly estimated fluxes shown in Fig. \ref{fig:SOExtrapolation} over the year, we find from this estimate that STT events may be responsible for at least $3.1 \times10^{16}$~molecules cm$^{-2}$ }\DIFdelend \DIFaddbegin \DIFadd{between 30$^{\circ}$ S and 60$^{\circ}$ S 210~Tg }\DIFaddend yr$^{-1}$ \DIFdelbegin \DIFdel{of the tropospheric ozone over the Southern Ocean, equivalent to 2.48~Tg yr$^{-1}$.
...
\DIFdel{of this global net downward flux can be attributed to STT events in the Southern Ocean region, a contribution that is likely too low.
  This result is only marginally sensitive to the latitude range chosen to represent the Southern Ocean (35$^{\circ}$ S-75}\DIFdelend \DIFaddbegin \DIFadd{.
  \mbox{%DIFAUXCMD
\citet{Liu2016}
}%DIFAUXCMD
model the upper tropospheric ozone and it's source (emissions/lightning/stratospheric) over the Atlantic ocean between 30}\DIFaddend $^{\circ}$ \DIFdelbegin \DIFdel{S): changing this range by 5}\DIFdelend \DIFaddbegin \DIFadd{S and 45}\DIFaddend $^{\circ}$ \DIFdelbegin \DIFdel{in either direction at either end of the range changes $\Omega_{SO_{O_3}}$ by -7 to +9}%DIFDELCMD < \%%%%
\DIFdel{, with even smaller impacts on the overall flux.
  }\DIFdelend \DIFaddbegin \DIFadd{S, and suggest that most of this is transported from the stratosphere from March to September, which is when the subtropical jet system is strongest.
  TODO: Small mention of supplementary results, with notes about simplistic nature of extrapolating over the whole SO.
  }

  \DIFaddend As discussed in Sect. \ref{sec:sensitivity}, there are several uncertainties in our method that are likely to lead to a low bias in the overall extrapolated flux, including a conservative estimate of flux within each event and a process that excludes potential events which are too close to the tropopause or to transported smoke plumes.
...
%DIFDELCMD <   %%%
\DIFdelend %It's also worth noting that our flux estimate only detects STT events that satisfy specific criteria, and may therefore be unsuitable for calculating more general STT fluxes.
...
%DIFDELCMD <   %%%
\DIFdelend %Considering the individual event contributions, \citet{Terao2008} estimate much higher STT impacts using a global CTM with a stratospheric ozone tracer; where 30--40\% of the 500~hPa ozone is due to STT.
  ...
  \DIFdelbegin \DIFdel{If we we assume a fractional ozone impact due to each event STT event of $I$=35}%DIFDELCMD < \% %%%
\DIFdel{based on their results (rather than our calculation of $I$=2-4}%DIFDELCMD < \%%%%
\DIFdel{), our estimate of the net downward ozone STT flux from Southern Ocean STT events increases by an order of magnitude to $\sim$29.5~Tg yr$^{-1}$, equivalent to 39}%DIFDELCMD < \% %%%
