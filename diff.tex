...
%DIF DEL ACPD_version2/OzonePaper.tex   Wed Jun 21 13:59:39 2017
%DIF ADD OzonePaper.tex                 Tue Aug  1 09:31:53 2017
%% ---------------------------------
...
%\input{Text/OzoneAbstract.tex}
  ...
  Using a bandpass filter on 7--9 years of ozone profiles from each site provides clear detection of STT events, with maximum occurrences during summer and minimum during winter \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend all three sites.
  The majority of tropospheric ozone enhancements \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{owing to }\DIFaddend STT events occur within 2.5~km and 3~km of the tropopause at Davis, and Macquarie Island\DIFaddbegin \DIFadd{, respectively}\DIFaddend .
...
%\section{Introduction}
  ...
  Another \DIFdelbegin \DIFdel{important region of }\DIFdelend \DIFaddbegin \DIFadd{hotspot for }\DIFaddend STT is the Middle East, where surface ozone exceeds values of 80~ppbv in summer, with \DIFaddbegin \DIFadd{a stratospheric contribution of }\DIFaddend 10~\DIFdelbegin \DIFdel{ppbv from STT contribution }\DIFdelend \DIFaddbegin \DIFadd{ppb }\DIFaddend \citep{Lelieveld2009}.
  ...
  Lower stratospheric and upper tropospheric ozone concentrations are highly correlated\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Terao2008}}%DIFAUXCMD
}\DIFdelend , suggesting mixing across the tropopause mainly associated with the jet streams over the \DIFdelbegin \DIFdel{ocean}\DIFdelend \DIFaddbegin \DIFadd{Atlantic and Pacific oceans \mbox{%DIFAUXCMD
\citep{Terao2008}}%DIFAUXCMD
}\DIFaddend .
  ...
  \DIFdelbegin \DIFdel{A big influence on the high surface ozone concentrations }\DIFdelend \DIFaddbegin \DIFadd{The summertime pool of high tropospheric ozone }\DIFaddend over the eastern Mediterranean (EM) \DIFdelbegin \DIFdel{can be attributed to stratospheric mixing and anticyclonic subsidence \mbox{%DIFAUXCMD
\citep{Zanis2014, Akritidis2016}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{is mainly attributed to the downward ozone transport, as a result of the enhanced subsidence \mbox{%DIFAUXCMD
\citep{Zanis2014} }%DIFAUXCMD
and the tropopause fold activity \mbox{%DIFAUXCMD
\citep{Akritidis2016} }%DIFAUXCMD
over the region}\DIFaddend .
  The EM \DIFdelbegin \DIFdel{frequently shows }\DIFdelend \DIFaddbegin \DIFadd{exhibits }\DIFaddend a summer maximum of \DIFdelbegin \DIFdel{stratospheric }\DIFdelend subsidence, which sits between 20$^{\circ}$E and 35$^{\circ}$E and 31$^{\circ}$N to 39$^{\circ}$N, while zonally most subtropical tropopause folds occur during winter \citep[][and references therein]{Tyrlis2014}.
  ...
  The long range transport of enhanced ozone can be facilitated by \DIFdelbegin \DIFdel{shear }\DIFdelend upper tropospheric winds, with remarkably little convective mixing, as shown by \cite{Trickl2014} who measure STT air masses two days and thousands of kilometres from their source.
  ...
  \DIFdelbegin \DIFdel{Section \ref{sec:DataMethods} describes }\DIFdelend \DIFaddbegin \DIFadd{In Section \ref{sec:DataMethods} we describe }\DIFaddend the observations and methods used to identify STT events and to relate STT occurrence to meteorological events. 
  We \DIFdelbegin \DIFdel{describe }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend how possible biomass burning smoke plume influence is detected and handled, and we introduce the GEOS-Chem model which is used for ozone flux estimation.
  \DIFdelbegin \DIFdel{Section \ref{sec:eventclimatologies} describes }\DIFdelend \DIFaddbegin \DIFadd{Within Section \ref{sec:eventclimatologies} we show }\DIFaddend the seasonality, altitude, depth, and frequency of detected STT events, along with a comparison of our findings to other literature where possible. 
  \DIFdelbegin \DIFdel{Section \ref{sec:ModelComparison} analyses }\DIFdelend \DIFaddbegin \DIFadd{In Section \ref{sec:ModelComparison} we analyse }\DIFaddend how well GEOS-Chem captures the tropospheric ozone seasonality and quantity near our three sites.
  \DIFdelbegin \DIFdel{Section \ref{sec:STTevents} uses }\DIFdelend \DIFaddbegin \DIFadd{In Section \ref{sec:STTevents} an extrapolation of }\DIFaddend the STT detection frequencies along with GEOS-Chem monthly tropospheric ozone columns \DIFaddbegin \DIFadd{is used }\DIFaddend to estimate STT ozone flux near our three sites. 
...
\section{Data and Methods}
  ...
  \label{sec:ozonesondes}
    ...
    The ozone tropopause is defined as the lowest altitude satisfying the following three conditions for the ozone mixing ratio (OMR) \citep{Bethan1996}:
     ...
     \end{enumerate} 
    The ozone tropopause may misdiagnose the real tropopause altitude during stratosphere-troposphere exchange; however, it is useful at polar latitudes in winter, where the lapse-rate definition may result in artificially high \DIFdelbegin \DIFdel{tropopause values }\DIFdelend \DIFaddbegin \DIFadd{values for tropopause height }\DIFaddend \citep{Bethan1996, Tomikawa2009, Alexander2013}.
    We require lapse rate \DIFaddbegin \DIFadd{defined }\DIFaddend tropopauses to be at a minimum of 4~km altitude. %This is due to several temperature inversions causing low misdiagnosed tropopauses.
    Another commonly used tropopause definition \DIFdelbegin \DIFdel{(the }\DIFdelend \DIFaddbegin \DIFadd{is determined with the use of PV (}\DIFaddend dynamical tropopause)\DIFdelbegin \DIFdel{is determined from the potential vorticity unit (PVU ; }\DIFdelend \DIFaddbegin \DIFadd{.
    In the extra-tropics the isosurface where PV$=2$~PVU (}\DIFaddend 1~PVU$= 10^{-6}$~m$^2$ s$^{-1}$ K kg$^{-1}$) \DIFdelbegin \DIFdel{.
    In the extra-tropics the isosurface where PVU$=2$ }\DIFdelend is often used \DIFdelbegin \DIFdel{, which allows a }\DIFdelend \DIFaddbegin \DIFadd{to define the tropopause, allowing the }\DIFaddend 3D \DIFdelbegin \DIFdel{view of }\DIFdelend \DIFaddbegin \DIFadd{representation of tropopause }\DIFaddend folds and other tropopause features in a sufficiently resolved model \citep{Skerlak2014,Tyrlis2014}.
    The \DIFdelbegin \DIFdel{PVU }\DIFdelend \DIFaddbegin \DIFadd{PV }\DIFaddend is not calculable using the ozonesonde measurements alone, so in this work the ozone tropopause is used when determining STT events or measured tropopause altitude.
...
\section{STT event climatologies}
  ...
  This seasonality is not clear in the recent ERA-Interim tropopause fold analysis performed by \citet{Skerlak2015}, where a winter maximum of \DIFdelbegin \DIFdel{ozone }\DIFdelend \DIFaddbegin \DIFadd{tropopause }\DIFaddend fold frequency ($\sim 0.5\%$ more folds in winter) over Australia can be seen to the north of Melbourne.
  Their work seems to show slightly higher fold frequencies over Melbourne in summer \citep[][Fig. 5]{Skerlak2015}, \DIFdelbegin \DIFdel{which agrees with our ozonesonde measurements}\DIFdelend \DIFaddbegin \DIFadd{however not to the same extent that our summer peak suggests}\DIFaddend .
  ...
  \begin{figure}[t]
    %DIF < \includegraphics[width=0.99\columnwidth]{figures/summary_altitude.png}
    %DIF >  fig 7
    %DIF >  done in examine_stations I think
    \includegraphics[width=12cm]{figures/summary_altitude.png}
    \caption{The distribution of STT \DIFdelbeginFL \DIFdelFL{event altitude }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{events' altitudes }\DIFaddendFL at Davis (top), Macquarie Island (middle), and Melbourne (bottom), determined as described in the text.
  ...
  \begin{figure}[t]
    ...
    \caption{The distribution of STT \DIFdelbeginFL \DIFdelFL{event depth}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{events' depths}\DIFaddendFL , defined as the distance from the event to the tropopause, at Davis (top), Macquarie Island (middle), and Melbourne (bottom), determined as described in the text.
    Events are coloured as described in Fig. \ref{fig:SummarySeasonality}.}
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \subsection{Results}
    ...
    \begin{figure}
      ...
      %DIF >  Figure from examine_stations.py  ->  flux_plots()
  ...
  \subsection{Comparison to literature}
    ...
    \cite{Roelofs1997} \DIFdelbegin \DIFdel{use a model with a stratospheric ozone tracer }\DIFdelend \DIFaddbegin \DIFadd{used a model carrying a tracer for stratospheric ozone }\DIFaddend to estimate STT impacts. 
