...
\section{Simulation of southern mid-latitude ozone columns}
  ...
  \end{equation*}
  where $z$ is the altitude (GPH), $TP$ is the altitude at the tropopause, $T$ is the temperature, $R$ is the ideal gas constant, and $N_{Av}$ is Avogadro's number.
...
\section{Stratosphere-to-troposphere ozone flux from STT events}
  ...
  \begin{figure*}[t]
    ...
    \caption{(Top) The three quantities used to calculate the total Southern Ocean ozone flux from STT events.
      The tropospheric ozone column $\Omega_{SO_{O_3}}$ (black, left axis) is from GEOS-Chem, while the STT fraction $f$ (blue, right axis) and Impact $I$ (purple, right axis) are from the ozonesonde measurements.
      The STT fraction is multiplied by 3 to bettershow the seasonality.
