...
\begin{abstract}
  ...
  Using an assumed STT impact (of 30\%) over the southern ocean rather than our conservative calculation increases the flux estimate to $\sim$21.38~Tg yr$^{-1}$.
...
\section{Conclusions}
  ...
  Using reduced major axis regression (which does not assume that there is no error in either X or Y) there is a near one to one slope at all three sites, with or without removing the seasonality of both GEOS-Chem and the ozonesondes.
